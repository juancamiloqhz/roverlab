...
function PlanetaryArea({ snapshot, fullWorld }: { snapshot: ExpeditionSnapshot; fullWorld: FullWorldView | null }) {
  ...
  const regions = new Map<string, { x: number; z: number; count: number }>();
  for (const cell of terrain) {
    if (!cell.region) continue;
    const center = regions.get(cell.region) ?? { x: 0, z: 0, count: 0 };
    center.x += cell.position.x; center.z += cell.position.z; center.count++;
    regions.set(cell.region, center);
  }
  ...
  return (
    <group>
      ...
      {terrain.map(cell => {
        ...
        return <group key={cell.id} position={[x, 0, z]}>
          <mesh receiveShadow rotation={[-Math.PI / 2, 0, 0]}>
            <planeGeometry args={[0.98, 0.98]} /><meshStandardMaterial color={current ? rough ? '#ac8058' : cell.region === 'Northern highlands' ? '#b6b2aa' : cell.region === 'Eastern volcanic field' ? '#b68b76' : '#d8b085' : rough ? '#716851' : '#8b8572'} roughness={1} />
          </mesh>
      ...
      {[...regions].map(([name, center]) => <Html key={name} position={[center.x / center.count, 0.15, center.z / center.count]} center zIndexRange={[7, 0]}>
        <span className="scene-label region-label">{name}</span>
      </Html>)}
      {samples.map(sample => {
        ...
        return <group key={sample.id} position={[sample.position.x, 0.18, sample.position.z]}>
          ...
          <Html position={[0, 0.7, 0]} center zIndexRange={[9, 0]}><span className={`scene-label sample-label ${current ? '' : 'remembered'}`}>{sample.label}<span className={fullWorld ? 'debug-knowledge' : undefined}> · {knowledgeLabel(sample.id)}</span></span></Html>
        </group>;
      })}
      ...
      {base && <group position={[base.position.x, 0.02, base.position.z]}>
        ...
        <Html position={[-1.7, 2.65, 0]} center zIndexRange={[10, 0]}><span className={`scene-label base-label ${currentIds.has(base.id) ? '' : 'remembered'}`}>Base · 01{currentIds.has(base.id) ? '' : ' · Remembered'}</span></Html>
      </group>}
    </group>
  );
}
  ...
  return (
    <section className="scene" aria-label="Planetary scene">
      ...
      <Canvas shadows camera={{ position: [29, 27, 34], fov: 43 }} dpr={[1, 2]} fallback={<p className="webgl-fallback">A WebGL-capable browser is needed to display the planetary scene.</p>}>
        ...
        <directionalLight castShadow position={[-5, 22, 4]} intensity={3} shadow-mapSize={[2048, 2048]} shadow-camera-left={-50} shadow-camera-right={50} shadow-camera-top={50} shadow-camera-bottom={-50} shadow-normalBias={0.04} />
