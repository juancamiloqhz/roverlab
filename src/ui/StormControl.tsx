...
export function StormControl({ snapshot, dispatch }: { snapshot: ExpeditionSnapshot; dispatch(command: ExpeditionCommand): void }) {
  const scheduled = snapshot.interventions?.schedule.events.some(event => event.kind === 'storm');
  ...
  return <section className="storm-control" aria-label="Dust storm controls">
    ...
    <button className="secondary" disabled={scheduled || !snapshot.stormAvailable || snapshot.stormIntroduced || snapshot.status === 'ended'} onClick={() => dispatch({ type: 'introduce-storm' })}>Introduce dust storm</button>
    {scheduled && !snapshot.stormIntroduced && <p>The storm is reserved by the intervention schedule.</p>}
    {!scheduled && !snapshot.stormIntroduced && <p>Introduce one temporary storm per expedition. Its region and duration become known after detection.</p>}
