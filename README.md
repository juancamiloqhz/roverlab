...
Edits record both their request time and their application at a safe action boundary. The interface distinguishes requested and effective preferences, superseded changes, and changes left unapplied when an expedition ends. The objective, rubric, and physical simulation remain fixed. Both controllers receive the same effective preferences and knowledge. The baseline still uses its existing fixed strategy; ticket 05 adds its interpretation of preset settings. Free-text experiments disclose that the baseline cannot interpret arbitrary instructions and are not labeled matched-priority benchmarks.
