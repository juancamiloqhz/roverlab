...
[Ticket 06](.scratch/first-playable-release/issues/06-run-a-typesafe-controlled-expedition.md) adds a selectable TypeSafe controller, a local Bun backend, returned Choice probabilities, bounded inference, and attempt/latency accounting. [Ticket 07](.scratch/first-playable-release/issues/07-recover-explicitly-from-inference-failures.md) adds explicit Retry and baseline continuation after inference failure, with controller transitions in the timeline and results. The baseline remains the default and requires no key or backend. The remaining [first playable release tickets](.scratch/first-playable-release/issues/) add storms, observation aids, and saved records/replay.
